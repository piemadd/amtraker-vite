...
const AmtrakerMap = () => {
  ...
  const setResultsAndRefreshMap = (showAllState, currentQuery) => {
    ...
    const finalFilter =
      showAllState && currentQuery.length == 0 // either adding a filter to filter by saved train ids or to allow all trains to be displayed
        ? ["any", true]
        : ["any", ...actualNewResults.map((n) => ["==", "id", n.trainID])];
        ...
        features: Object.values(data)
          ...
          .map((train) => {
            return {
              type: "Feature",
              id: train.trainID,
              properties: { trainNum: train.trainNum, trainID: train.trainID, routeName: train.routeName },
      ...
      mapRef.current.on("load", async () => {
        mapRef.current.on("click", (e) => {
          ...
          if (f.length > 1) {
            ...
            const finalItemsWithData = finalItems.map((item) => {
              if (item.layer.id == "stations") {
                return {
                  ...item,
                  properties: window.dataManager.getStationSync(item.properties.code)[item.properties.code]
                };
              } else if (item.layer.id == "trains") {
                return {
                  ...item,
                  properties: window.dataManager.getTrainSync(item.properties.trainID)[item.properties.trainNum][0]
                };
              } else {
                return item;
              }
          ...
          switch (feature.layer.id) {
            case "trains":
              const train = dataManager.getTrainSync(feature.properties.trainID)[feature.properties.trainNum][0];
              ...
              break;
            case "stations":
              const station = dataManager.getStationSync(feature.properties.code)[feature.properties.code];
                      ...
                      case "trains":
                        return (
                          <div
                            ...
                            onClick={(e) => {
                              const train = dataManager.getTrainSync(item.properties.trainID)[
                                item.properties.trainNum
                              ][0];
                      ...
                      case "stations":
                        return (
                          <div
                            ...
                            style={{ cursor: "pointer" }}
                            onClick={(e) => {
                              const station = dataManager.getStationSync(feature.properties.code)[
